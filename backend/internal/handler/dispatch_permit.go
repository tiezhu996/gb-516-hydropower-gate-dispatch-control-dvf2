...
func (h *DispatchPermitHandler) issue(c *gin.Context) {
	id, ok := parseID(c)
	if !ok {
		return
	}
	var input dto.IssueDispatchPermit
	if err := c.ShouldBindJSON(&input); err != nil {
		util.Fail(c, http.StatusBadRequest, "invalid_request", err.Error())
		return
	}
	item, err := h.service.Issue(c.Request.Context(), id, input, actorFromContext(c), roleFromContext(c), requestIDFromContext(c))
	if err != nil {
		handleError(c, err)
		return
	}
...
func (h *DispatchPermitHandler) revoke(c *gin.Context) {
	id, ok := parseID(c)
	if !ok {
		return
	}
	var input dto.RevokeDispatchPermit
	if err := c.ShouldBindJSON(&input); err != nil {
		util.Fail(c, http.StatusBadRequest, "invalid_request", err.Error())
		return
	}
	item, err := h.service.Revoke(c.Request.Context(), id, input, actorFromContext(c), roleFromContext(c), requestIDFromContext(c))
	if err != nil {
		handleError(c, err)
		return
	}
